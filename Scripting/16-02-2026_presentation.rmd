...
total_df <- metadf %>%
    filter(Ann_lvl_2 %in% c("Tregs")) %>%
    group_by(sample_name) %>%
    summarise(Treg_count = sum(count)) %>%
    as.data.frame() %>%
    right_join(total_df, by = "sample_name")
...
total_df[["Treg_prop"]] <- (total_df[["Treg_count"]] / total_df[["sample_count"]])
...
```{r}
columns_to_check_continous <- unlist(lapply(colnames(res_mlm_final)[7:48], \(col){
...
df_list <- lapply(c('IDH_status','tumour_abbreviated'), \(column){
    print(column)
    data <- res_mlm_final
    data$condition <- data[[column]]
    data %>%
        pivot_longer(
            cols = columns_to_check_continous,
            names_to = "variable",
            values_to = "value"
        ) %>% filter(value < 100000) %>% 
...
result_df %>%
    filter(abs(cor) > 0.4) %>%
    filter(p < 0.05) %>% group_by(variable) %>% summarise(count = n()) %>% arrange(desc(count)) %>% pull(variable,count)
    gt::gt()
```
...
plot <- res_mlm_final %>% ggpubr::ggscatter(
        x = 'Microglia_Macro_mono', y = "score",
        add = "reg.line",
...
plot1 <- ggpubr::facet(plot,
    facet.by = c("source", "tumour_abbreviated"), scales = "free_y", short.panel.labs = TRUE, # Allow long labels in panels
    panel.labs.background = list(fill = "grey", color = "grey"), panel.labs.font = list(size = 13, color = "black")
)
plot1

plot <- res_mlm_final %>% ggpubr::ggscatter(
        x = 'Macro_mono_prop', y = "score",
        add = "reg.line",
        add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line    conf.int = TRUE, # Add confidence interval
        cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
        conf.int = TRUE, # Add confidence interval
...
plot2 <- ggpubr::facet(plot,
        facet.by = c("source", "tumour_abbreviated"), scales = "free", short.panel.labs = TRUE, # Allow long labels in panels
        panel.labs.background = list(fill = "darkgrey", color = "black"), panel.labs.font = list(size = 15, color = "black")
        # panel.labs.background = list(fill = "skyblue", color = "black")
    )
plot2
plot <- res_mlm_final %>% ggpubr::ggscatter(
        x = 'Microglia_prop', y = "score",
        add = "reg.line",
        add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line    conf.int = TRUE, # Add confidence interval
        cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
        conf.int = TRUE, # Add confidence interval
...
plot3 <- ggpubr::facet(plot,
    facet.by = c("source", "tumour_abbreviated"), scales = "free", short.panel.labs = TRUE, # Allow long labels in panels
    panel.labs.background = list(fill = "darkgrey", color = "black"), panel.labs.font = list(size = 15, color = "black")
    # panel.labs.background = list(fill = "skyblue", color = "black")
)
plot3 <- plot3 + theme( axis.title = element_text(face = 'bold',size = 12))
plot3 + plot_annotation(title = 'Microglia presence determines environment functionality', theme = theme(plot.title = element_text(face = 'bold', size = 15) ))
