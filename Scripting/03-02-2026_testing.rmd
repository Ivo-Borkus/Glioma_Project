...
# p <- VlnPlot(obj, features = c(Ucell_cols[14:18],'infiltration-residence'), group.by = 'Ann_lvl_2', ncol = 2,raster = T, pt.size = 0) + x_axis_theme
# p3 <- VlnPlot(obj, features ='infiltration-residence', group.by = 'Ann_lvl_2', ncol = 2,raster = T, pt.size = 0) + x_axis_theme
...
reduction_name <- 'M_M'
obj <- qs_read(here(paste0('Glioma_Project/03_seurat_objects/Processed_',reduction_name,'.qs2')))
obj$`inf-res` <- (scales::rescale(obj$Infiltrating_UCell)  - scales::rescale(obj$Resident_UCell)) # so they are scaled
...
p_overview_tumour <- res_mlm %>% ggplot(mapping = aes(x = tumour_abbreviated, y = score))+
    ...
    theme_bw() + x_axis_theme 
pdf(here('Glioma_Project/04_figs/significance_testing.pdf'), width = 9, height = 5)
p_overview_IDH
p_overview_tumour
dev.off()
...
seurat_obj_clean_P <- seurat_processing_manual(seurat_obj_clean, reduction_name = 'clean')
