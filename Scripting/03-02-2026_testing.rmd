...
p_overview_tumour <- res_mlm %>% ggplot(mapping = aes(x = tumour_abbreviated, y = score))+
    ...
    theme_bw() + x_axis_theme 
pdf(here('Glioma_Project/04_figs/02_significance_testing.pdf'), width = 9, height = 5)
p_overview_IDH
p_overview_tumour
dev.off()
...
p2 <-.seurat_dimplot(seurat_obj_clean_P, reduction = 'umap.harmony.clean',label = T, title = 'Ann_lvl_2',group = 'Ann_lvl_2', pt.size = 1) #+ theme(legend.position = 'none')
metadf <- seurat_obj_clean@meta.data %>% group_by(Ann_lvl_2,Ann_lvl_1H,sample_name) %>% summarise(count = n()) %>% group_by(sample_name) %>% mutate(percentage = (count / sum(count) )* 100 )
metadf$Ann_lvl_1H %>% table()
seurat_obj_clean@meta.data[,c('sample_name','Ann_lvl_1H')] %>% table()

## Ratios to compute:
## T-cells to Myeloid ratio
## T-cells to DC fraction
## Microglia to Macro/Mono 

metadf <- seurat_obj_clean@meta.data %>% group_by(Ann_lvl_2,Ann_lvl_1H,sample_name) %>% summarise(count = n())
# Creating the totals.
total_df <- metadf %>%  group_by(sample_name) %>% summarise(sample_count = sum(count)) %>% as.data.frame()
total_df <- metadf %>% filter(Ann_lvl_1H %in% c('T-CD8+','T-CD4+','T-other','NK','T-Prolif')) %>% group_by(sample_name) %>% summarise(Tcellcount = sum(count)) %>% as.data.frame() %>% right_join(total_df, by = 'sample_name')
total_df <- metadf %>% filter(!Ann_lvl_1H %in% c('T-CD8+','T-CD4+','T-other','NK','T-Prolif')) %>% group_by(sample_name) %>% summarise(Mcellcount = sum(count)) %>% as.data.frame() %>% right_join(total_df, by = 'sample_name')


total_df <- metadf %>% filter(Ann_lvl_1H %in% c('DCs')) %>% group_by(sample_name) %>% summarise(DC_count = sum(count)) %>% as.data.frame() %>% right_join(total_df, by = 'sample_name')
total_df <- metadf %>% filter(Ann_lvl_1H %in% c('Microglia')) %>% group_by(sample_name) %>% summarise(Microglia = sum(count)) %>% as.data.frame() %>% right_join(total_df, by = 'sample_name')
total_df <- metadf %>% filter(Ann_lvl_1H %in% c('Macrophage','Monocyte')) %>% group_by(sample_name) %>% summarise(Macro_mono = sum(count)) %>% as.data.frame() %>% right_join(total_df, by = 'sample_name')
...
p <- lapply(colnames(total_df)[2:13], \(col){
    plot <- res_mlm_final %>% ggpubr::ggscatter(x = as.character(col), y = 'score',
        add = 'reg.line',
    ...
    plot <- ggpubr::facet(plot, facet.by = c('source','tumour_abbreviated'),scales = 'free',  short.panel.labs = T,   # Allow long labels in panels
        panel.labs.background = list(fill = "grey", color = "grey"), panel.labs.font = list(size = 13, color = 'black'))
    return(plot)
})
pdf(here('Glioma_Project/04_figs/significance_ratios_SUBtype.pdf'), width = 8, height = 15)
p
dev.off()


p2 <- lapply(colnames(total_df)[2:13], \(col){
    plot <- res_mlm_final %>% ggpubr::ggscatter(x = as.character(col), y = 'score',
        add = 'reg.line',
        add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line    conf.int = TRUE, # Add confidence interval
        cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
    ...
    plot <- ggpubr::facet(plot, facet.by = c('source','IDH_status'),scales = 'free',  short.panel.labs = TRUE,  # Allow long labels in panels
        panel.labs.background = list(fill = "grey", color = "grey"), panel.labs.font = list(size = 13, color = 'black'))
    return(plot)
})
pdf(here('Glioma_Project/04_figs/significance_ratios_IDH.pdf'), width = 8, height = 12)
p2
dev.off()
...
testing_object %>% as.data.frame() %>% filter(p.adj < 0.05) %>% gt::gt()

columns_to_check_continous <- unlist(lapply(colnames(res_mlm_final)[7:38], \(col){
    vec <- res_mlm_final[[col]]
...
result_dfs <- lapply(columns_to_check_continous, \(col){
    ...
    final_df$comparison <- col
    return(final_df)
})
result_df <- do.call(rbind,result_dfs)
result_df %>% filter(R.squared > 0.4) %>% filter(p_value < 0.05 )
```

```{r}
# testing
colnames(res_mlm_final)
plot <- res_mlm_final %>% ggpubr::ggscatter(x = as.character('DC_count_Microglia'), y = 'score',
        add = 'reg.line',
        add.params = list(color = "blue", fill = "lightgray"), # Customize reg. line    conf.int = TRUE, # Add confidence interval
        cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
        conf.int = TRUE, # Add confidence interval
...
cutoff_infl_p <- median(obj$True_Inflammation) + sd(obj$True_Inflammation)
cutoff_infl_n <- median(obj$True_Inflammation) - sd(obj$True_Inflammation)
cutoff_infi_p <- median(obj$True_Infiltration) + sd(obj$True_Infiltration)
cutoff_infi_n <- median(obj$True_Infiltration) - sd(obj$True_Infiltration)
obj@meta.data <- obj@meta.data %>% mutate(Infiltration_cells = case_when(
...
pdf(here('Glioma_Project/04_figs/Test_signature.pdf'),width = 15, height = 10)
plot_list <- lapply(old_cols, \(i){
    p <- ggplot(obj@meta.data, mapping = aes(x = Ann_lvl_2, y = !!sym(i), fill = Ann_lvl_1H))+ ggrastr::rasterise(geom_violin()) + ggrastr::rasterise(geom_boxplot(width = 0.1, alpha = 0.5))+
    x_axis_theme + ggtitle(i)
})
...
lapply(old_cols, \(i){
    p <- ggplot(obj@meta.data, mapping = aes(x = Inflammatory_cells, y = !!sym(i)))+ ggrastr::rasterise(geom_violin()) + ggrastr::rasterise(geom_boxplot(width = 0.1, alpha = 0.5))+
    x_axis_theme + ggtitle(i)
...
## Infiltratin Old sig
lapply(old_cols, \(i){
    p <- ggplot(obj@meta.data, mapping = aes(x = Infiltration_cells, y = !!sym(i)))+ ggrastr::rasterise(geom_violin()) + ggrastr::rasterise(geom_boxplot(width = 0.1, alpha = 0.5))+
    x_axis_theme + ggtitle(i)
