...
p_scatter <- ggplot(data = obj@meta.data, mapping = aes(x = `Microglial inflammatory_UCell`, y = `Systemic inflammatory_UCell`, colour = Ann_lvl_1H)) + ggrastr::rasterise(geom_point()) + 
    annotate('text', x = 0.1,y = 0.8, label = paste0('R-squared: ', as.character(round(summary(model)$r.squared,2))))
cols <- c(Ucell_cols[15:16],'Infiltration-Residence')
plot_list <- lapply(cols, \(i){
...
p <- plot_list |> wrap_plots(ncol = 2) + plot_layout(guides = 'collect',axes = 'collect')

cols_2 <-  c(Ucell_cols[14:18],'Infiltration-Residence')
plot_list_2 <- lapply(cols_2, \(i){
    p <- ggplot(obj@meta.data, mapping = aes(x = Ann_lvl_2, y = !!sym(i), fill = Ann_lvl_1H))+ ggrastr::rasterise(geom_violin()) + ggrastr::rasterise(geom_boxplot(width = 0.1, alpha = 0.5))+
    x_axis_theme + ggtitle(i)
})
pextra <- plot_list_2 |> wrap_plots(ncol = 2) + plot_layout(guides = 'collect',axes = 'collect')
...
pqc <- plot_list_3 |> wrap_plots(ncol = 2) + plot_layout(guides = 'collect',axes = 'collect')

pdf(here('Glioma_Project/04_figs/01_Signature_violins.pdf'), width = 15, height = 10)
p
pextra
pqc
dev.off()
