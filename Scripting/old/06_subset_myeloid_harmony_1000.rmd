...
batch_cols <- c(
    "Sample", "tumour_full", "tumour_abbreviated", "grade", "Cell_Type", "Cell_Subtype", "sample_name", "IDH_status",
...
.seurat_dimplot(seurat_obj, reduction = reduction_umap, group = "Sample", label = TRUE)
.seurat_dimplot(seurat_obj, reduction = reduction_umap, group = "Ann_lvl_2M", label = TRUE)

# ggsave(here(fig_path, paste0("sample_plot", reduction_name, "_.png")))

dittoSeq::dittoBarPlot(seurat_obj, var = "Sample", group.by = "Ann_lvl_2M") # , var.labels.reorder = cluster_order)
ggsave(here(fig_path, paste0("Barplot_Populations", reduction_name, "_.png")))
dittoSeq::dittoBarPlot(seurat_obj, var = "Ann_lvl_2M", group.by = "Sample") # , var.labels.reorder = cluster_order)
ggsave(here(fig_path, paste0("Barplot_Sample", reduction_name, "_.png")))
...
plot_1 <- DimPlot(seurat_obj, reduction = reduction_umap, group.by = resolution, label = T) & NoLegend() & NoAxes()
ggsave(here(fig_path, paste0(resolution, "_", reduction_name, "_Dimplot.png")))
plot_2 <- DimPlot(seurat_obj, reduction = reduction_umap, group.by = "Ann_lvl_2M", label = T) & NoLegend() & NoAxes()
...
new.cluster.ids <- c(
    "Mg_Inflam", "Ma-Inflam", "Myeloid_Prolif", "Mono-Classic", "DCs_CD1C+", "Myeloid_Prolif",
    "Mg_PLCG2", "Ma-Mt+", "Hb+", "pDCs", "Mast-cells", "Mg_Anti-Inflam", "TAMs", "Mg_classic", "BAMs", "Ma-IFN", "Mg_active", "Mg_PLCG2", "Mg_PLCG2"
)
...
seurat_obj[["Ann_lvl_2M"]] <- Idents(seurat_obj)
DimPlot(seurat_obj, reduction = reduction_umap, group.by = "Ann_lvl_2M", label = T) & NoAxes()
...
```{r plotting score, eval = F,results = 'asis', echo = F}
