...
manual_PC <- ggplot(plot_df, aes(cumu, pct, label = rank, color = rank < n_PCS)) +
    geom_text() +
    geom_vline(xintercept = 90, color = "grey") +
    geom_hline(yintercept = min(pct[pct > 2]), color = "grey") +
    theme_bw()

PCA_elbow_seurat <- ElbowPlot(seurat_obj, reduction = paste0("pca_", reduction_name), ndims = 200)
Genes_influence_PCA <- VizDimLoadings(seurat_obj,
    dims = 1:5, reduction = paste0("pca_", reduction_name), nfeatures = 15
)

# PCA_elbow_seurat <- ElbowPlot(seurat_obj, reduction = paste0("harmony_pca.", reduction_name), ndims = 200) # does not work
# Genes_influence_PCA <- VizDimLoadings(seurat_obj,
#     dims = 1:5, reduction = paste0("harmony_pca.", reduction_name), nfeatures = 15
# ) # Does not work

# seurat_obj@reductions$harmony_pca.Myeloid_Harmony_1000@feature.loadings # Empty
# seurat_obj@reductions$pca_Myeloid_Harmony_1000@feature.loadings # Not empty

# seurat_obj@reductions$harmony_pca.Myeloid_Harmony_1000@stdev # Empty
# seurat_obj@reductions$pca_Myeloid_Harmony_1000@stdev # Not empty

print(manual_PC)
print(PCA_elbow_seurat)
print(Genes_influence_PCA)
```
...
.tabsetter(plot_list_batch, overall_name = paste0("Batch plots: ", reduction_name), path = rmd_path)
```
...
## Setting gene signatures to use for validation
...
gene_list <- list(
    ...
    Cytotoxic = c("GNLY", "GZMA", "GZMB", "GZMK", "NKG7", "PRF1"),
    naive = c("CD3E", "CD3D", "CCR7", "IL7R", "SELL", "LEF1", "TCF7"),
    effector = c("CD3E", "CD3D", "KLRG1", "CX3CR1", "GZMK", "PRDM1", "EOMES"), # Differentiation markers)
    exhaust = c("LAG3", "NKG7", "HAVCR2", "CTLA4", "PDCD1", "TOX"),
    treg = c("IL2RA", "FOXP3"),
    NK = c("KLRF1", "NCAM1", "CD3E-", "CD3D+"),
    Gd = c("TRDC", "TRGC2", "TRAC-", "TRBC1-", "TRBC2-")
)
```

```{r add the signatures, eval = F}
...
DimPlot(seurat_obj, reduction = reduction_umap, group.by = "Ann_lvl_1", label = T) & NoAxes() & NoLegend()
...
colnames(top5_wide) <- make.names(new.cluster.ids, unique = TRUE)
...
FeaturePlot(seurat_obj, features = paste0(names(gene_list), "_UCell"), reduction = reduction_umap) & NoAxes()
```


```{r plotting score, results = 'asis', echo = F}
.tabsetter(plot_list_score, overall_name = "Scoring plots", path = rmd_path)
```
...
<!-- rmarkdown::render("Glioma_Project/Scripting/03_Integration.rmd", output_dir = "Glioma_Project/02_knits", output_file = paste0(Sys.Date(), "03_Integration_Glioma.html"))  -->
